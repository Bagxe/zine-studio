...
var FORMAT_INFO = {
  half: { label: "Half (5.5\xD78.5, saddle-stitch)", duplexOnly: true },
  quarter: { label: "Quarter (4.25\xD75.5)", supportsCopy: true },
...
var INK_LUM = 235;
var GUTTER_INK = 0.02;
var MIN_GUTTER_FRAC = 3e-3;
var MIN_BAND_FRAC = 0.02;
var SEAM_RATIO = 1.5;
var SEAM_MIN_SPAN = 0.5;
var SEAM_MAX = 8;
function luminance(p, o) {
  return 0.299 * p[o] + 0.587 * p[o + 1] + 0.114 * p[o + 2];
}
function inkProfile(pixels, w, h, axis) {
...
function countBands(profile) {
  ...
  while (changed) {
    ...
    while (i2 < dim) {
      ...
      const len = j - i2;
      if (v && len < minGutter && i2 > 0 && j < dim) {
        gutter.fill(false, i2, j);
        changed = true;
      } else if (!v && len < minBand) {
        gutter.fill(true, i2, j);
        changed = true;
      }
      i2 = j;
    }
...
function seamCount(pixels, w, h, axis) {
  const axisDim = axis === "x" ? w : h;
  const otherDim = axis === "x" ? h : w;
  if (axisDim < 16) return 1;
  const stride = Math.max(1, Math.floor(otherDim / 400));
  const e = new Float32Array(axisDim - 1);
...
var COMMON_MASK_RES = 2048;
function commonGutterBands(sides, axis) {
  const mask = new Uint8Array(COMMON_MASK_RES).fill(1);
...
function matchFormat(rows, cols, sheetW, sheetH, numSides) {
  ...
  candidates.sort(
    (a, b) => a.confidence === b.confidence ? 0 : a.confidence === "exact" ? -1 : 1
  );
  return {
    grid: { rows, cols },
    sheetPortrait,
